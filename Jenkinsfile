node{
 ...
 //   stage('Build + SonarQube analysis') {
  ...
  stage('Upload Artifacts'){
      bat label: '', script: '''curl -uadmin:password -T "C:\\Program Files (x86)\\Jenkins\\workspace\\.Net-Project_Pipeline\\BlogEngine\\BlogEngine.NET\\obj\\Release\\Package\\BlogEngine.NET.zip" "http://localhost:8081/artifactory/DOTNET-PROJECT/${buildName}/${buildNumber}/BlogEngine.NET.zip?properties=$build.number"
'''
  }
